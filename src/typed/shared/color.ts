...
// Ported from documents.js's src/model/color.ts, renamed LayoutColor -> Color: this package has no PDF/layout concept for the name to distinguish itself from, and "Color" matches ooxml.js's own unprefixed model-type convention (Run, Paragraph, Table, ...).
...
// Parses a 6-digit hex colour (OOXML's w:color/@w:val, a:srgbClr/@val), with or without a leading '#', into a Color. Throws on malformed input rather than substituting a default -- callers at the OOXML boundary are expected to have already validated the attribute is present.
