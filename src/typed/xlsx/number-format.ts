// A tokenizing classifier for Excel's number-format mini-language (the string carried by xl/styles.xml's own <numFmt formatCode="...">, and by ECMA-376 Part 1 SS18.8.30's own implied built-in table below). It answers exactly one question: what KIND of value does a numeric cell carrying this format actually hold -- a percentage, an amount of money, a date, a time of day, an elapsed duration, or a plain number? It is deliberately NOT a formatter: nothing here renders a value THROUGH a format code (that needs locale data, fill/alignment placeholder geometry, conditional-section evaluation, and colour handling none of this package's consumers have asked for), only classifies one. typed/xlsx/content.ts's readCellValue is the sole caller; typed/xlsx/styles.ts resolves a cell's own style index to the format code fed in here.
//
