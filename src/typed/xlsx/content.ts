...
// Package -> ContentDocument (kind: 'spreadsheet'): a SpreadsheetML reader built geometry- and print-settings-rich, matching readOds's own established bar in the sibling odf.js package (real column widths, row heights, hidden rows/columns, merged ranges, every cell value kind this format actually distinguishes, and a genuinely populated ContentSheetPrintSettings), rather than the lossy cell-values-only projection typed/xlsx.ts's own readXlsxWorkbook provides. Unlike readOds, this returns a full ContentDocument envelope directly (kind/metadata/sheets) rather than a bare {metadata, sheets} shape -- readXlsxContent and typed/xlsx/build.ts's buildXlsxPackageFromContent are designed as a matched read/write pair around ContentDocument specifically, so a caller can round-trip readXlsxContent(buildXlsxPackageFromContent(x)) without an extra wrapping/unwrapping step, and documents.js's own future ods<->xlsx bridge (bypassing PDF, the same way its existing odt<->docx/odp<->pptx bridges do) can treat this reader's own output as an already-correctly-shaped pivot value.
//
// This is the flat, content-level half of the xlsx read pair: readXlsx (typed/document-package.ts) wraps it into a tree-form DocumentPackage, which is the primary name. Nothing is lost either way -- a spreadsheet's tree is one sheet group per sheet -- so which one to call is purely which shape the caller works in.
//
